...
async function requireUser(req,env){const u=await user(req,env);if(!u)throw Object.assign(new Error('Authentication required'),{status:401});if(!(u.email_confirmed_at||u.phone_confirmed_at||u.confirmed_at))throw Object.assign(new Error('Confirmed account required'),{status:403});return u}
async function operatorAccess(env,u,tenantId){const rows=await sb(env,`operator_members?tenant_id=eq.${encodeURIComponent(tenantId)}&auth_user_id=eq.${encodeURIComponent(u.id)}&select=id`);return Boolean(rows?.length)}
