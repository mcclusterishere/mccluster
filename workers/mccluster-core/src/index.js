...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (!configured(env)) return fail(request, env, 'McCluster Core is not configured', 503);
      ...
      if (path === '/api' || path.startsWith('/api/')) {
        return fail(request, env, 'Whip product routes belong on mccluster-core. Merge workers/mccluster-core/src/whip handlers before serving /api/* from this Worker.', 503);
      }
      ...
      return fail(request, env, error.message || 'McCluster Core request failed', error.status || 500, error.detail);
    }
  }
};
