...
class OutlineEffect {
	...
	constructor( renderer, parameters = {} ) {
		...
		const vertexShader = [
			...
			// UPRISE PATCH: the stock line below computed pos2 without the
			// instance matrix, while <project_vertex> applies it to `pos`. On an
			// InstancedMesh with any rotation or scale the two lived in different
			// spaces, so the outline offset direction was wrong and the outlines
			// smeared. Push the offset point through the same instance transform.
			'	vec4 outlineLocal = vec4( skinned.xyz + normal, 1.0 );',
			'	#ifdef USE_INSTANCING',
			'		outlineLocal = instanceMatrix * outlineLocal;',
			'	#endif',
			'	vec4 pos2 = projectionMatrix * modelViewMatrix * outlineLocal;',
