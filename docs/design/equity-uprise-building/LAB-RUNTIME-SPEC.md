...
Step 2 connects the Step 1 runtime lifecycle to the canonical Step 4A electronics model.

The binding layer consumes the current Digital-to-Physical Asset Registry, electronics manifest, typed connections and existing 40-lab catalog. Every current lab target selector resolves deterministically to an exact asset ID, asset type, cable/connection family, the cable catalog, or whole-building electronics scope.

The sandbox maintains separate runtime state for canonical assets and typed connections. Baseline authority is immutable: fault injection changes only in-memory simulation state.

Implemented propagation includes:
- PoE / horizontal-link faults affecting modeled endpoints;
- access-switch failure through patch-panel and Cat6A endpoint paths;
- access-switch uplink degradation/failover;
- virtualization-host failure to hosted logical services;
- logical-service dependency degradation;
- BAS controller failure through BACnet MS/TP links to field sensors;
- access-controller reader-bus failure through OSDP links;
- IDF UPS failure into the corresponding access-switch/end-device domain;
- backbone fiber failures on OS2 uplinks;
- firewall HA / VPN effect state;
- IT/OT segmentation and rogue-device security-state faults;
- whole-building cross-system incident state.

Runtime inspections return current simulated selector state, and evidence export includes changed electronics state alongside the Step 1 event ledger.

### Safety boundary

Step 2 remains simulation-only:
- no shell/network/device commands;
- no protocol write operations;
- no credentials;
- no writes to real switches, BAS, access control, fire alarm, cameras, hypervisors, UPS equipment or B1 operational systems;
- any LIVE execution request remains rejected.

The Step 2 verifier proves all 40 labs bind, all 33 target selectors resolve, the complete current fault vocabulary has concrete effects, representative propagation chains work, replay is deterministic, reset restores baseline, inspection is state-backed and LIVE control remains rejected.

**Next:** Step 3 promotes the first serious CISA/IT-OT lab set into richer task-specific executable scenarios using this state engine.
