...
programs=json.loads((HERE/"core-v2-floor-programs.json").read_text());f1=next(x for x in programs["levels"] if x["level"]==1);f2=next(x for x in programs["levels"] if x["level"]==2);f3=next(x for x in programs["levels"] if x["level"]==3);f4=next(x for x in programs["levels"] if x["level"]==4)
check("Floor 1 reconciled",f1.get("design_maturity")=="reconciled-current-iterative-pass",str(f1.get("design_maturity")));check("Floor 2 reconciled",f2.get("design_maturity")=="reconciled-current-iterative-pass",str(f2.get("design_maturity")));check("Floor 3 reconciled",f3.get("design_maturity")=="reconciled-current-iterative-pass",str(f3.get("design_maturity")));check("Floor 4 reconciled",f4.get("design_maturity")=="reconciled-current-iterative-pass",str(f4.get("design_maturity")));check("Floors 5-7 provisional",all(x.get("design_maturity")=="pre-iterative-program-rewrite" for x in programs["levels"] if x["level"]>=5))
