...
check("Floor 1 reconciled",f1.get("design_maturity")=="reconciled-current-iterative-pass",str(f1.get("design_maturity")));check("Floors 2-7 provisional",all(x.get("design_maturity")=="pre-iterative-program-rewrite" for x in programs["levels"] if x["level"]>=2))
