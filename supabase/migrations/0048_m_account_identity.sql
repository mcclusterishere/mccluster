...
--
-- auth.users.id remains the canonical M UID. OAuth identities are attached
-- by Supabase Auth; this migration only records which McCluster app/org a
-- signed-in user touched and a random first-party installation id. The
...
begin
  ...
  where app_key = btrim(p_app_key) and enabled = true;
  if not found then
    raise exception 'unknown or disabled app' using errcode = '22023';
  end if;
  ...
  if not found then
    raise exception 'unknown or disabled organization' using errcode = '22023';
  end if;
  ...
  on conflict (user_id, app_id, org_id) do update
    set last_seen_at = now();
