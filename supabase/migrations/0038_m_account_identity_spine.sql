...
create policy platform_profiles_update_own
  on public.platform_profiles for update to authenticated
  using ((select auth.uid()) is not null and (select auth.uid()) = user_id)
...
create policy platform_user_devices_select_own
  on public.platform_user_devices for select to authenticated
  using ((select auth.uid()) is not null and (select auth.uid()) = user_id);

drop policy if exists platform_user_devices_insert_own on public.platform_user_devices;
create policy platform_user_devices_insert_own
  on public.platform_user_devices for insert to authenticated
  with check ((select auth.uid()) is not null and (select auth.uid()) = user_id);

drop policy if exists platform_user_devices_update_own on public.platform_user_devices;
create policy platform_user_devices_update_own
  on public.platform_user_devices for update to authenticated
  using ((select auth.uid()) is not null and (select auth.uid()) = user_id)
  with check ((select auth.uid()) is not null and (select auth.uid()) = user_id);

drop policy if exists platform_user_devices_delete_own on public.platform_user_devices;
create policy platform_user_devices_delete_own
  on public.platform_user_devices for delete to authenticated
  using ((select auth.uid()) is not null and (select auth.uid()) = user_id);
...
begin
  insert into public.platform_profiles (
    user_id, display_name, primary_email, phone, avatar_url, updated_at
  ) values (
    new.id,
...
create trigger platform_sync_profile_from_auth_trg
after insert or update of email, phone, raw_user_meta_data on auth.users
for each row execute function public.platform_sync_profile_from_auth();
