...
--
-- Production acquired the original outreach-oriented ops_signals table before
-- that DDL was captured in repository migration history. Reconstruct the exact
-- legacy base shape when replaying from a blank database, while remaining a
-- no-op for an existing production table. The rest of this migration promotes
-- that table into the canonical provider-neutral signal spine.
...
-- ops_signals.id is a bigint GENERATED BY DEFAULT AS IDENTITY column backed by
-- public.ops_signals_id_seq. Keep the internal writer functional while removing
-- the legacy direct sequence grants from client roles.
revoke all on sequence public.ops_signals_id_seq from public, anon, authenticated;
