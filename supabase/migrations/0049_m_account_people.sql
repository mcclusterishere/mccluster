...
begin
  for r in
    select u.id
    from auth.users u
    left join public.m_auth_user_links l on l.auth_user_id = u.id
...
declare
  v_m_uid uuid;
begin
  if exists (select 1 from public.m_auth_user_links where auth_user_id = new.id) then
    return new;
  end if;
  insert into public.m_people default values returning id into v_m_uid;
  insert into public.m_auth_user_links(auth_user_id, m_uid, link_method, is_primary)
...
create trigger m_auth_user_after_insert
after insert on auth.users
for each row execute function public.m_auth_user_after_insert();
...
create policy m_people_read_self on public.m_people
  ...
  using (exists (
    ...
    where l.m_uid = m_people.id and l.auth_user_id = auth.uid()
  ));
...
update public.m_devices d
set m_uid = l.m_uid
from public.m_auth_user_links l
where d.user_id = l.auth_user_id and d.m_uid is null;
...
create policy m_person_apps_read_self on public.m_person_apps
  ...
  using (exists (
    select 1 from public.m_auth_user_links l
    where l.m_uid = m_person_apps.m_uid and l.auth_user_id = auth.uid()
  ));
...
as $$
  with me as (
    select l.m_uid from public.m_auth_user_links l where l.auth_user_id = auth.uid()
  ), linked as (
    select l.auth_user_id from public.m_auth_user_links l, me where l.m_uid = me.m_uid
  )
  select coalesce(
    jsonb_agg(jsonb_build_object('id', i.id, 'provider', i.provider, 'provider_id', i.provider_id,
      'email', i.email, 'auth_user_id', i.user_id, 'created_at', i.created_at, 'last_sign_in_at', i.last_sign_in_at)
...
-- Deliberately service-role only. This is the controlled path for merging two
-- already-verified M identities; clients cannot call it themselves.
...
begin
  ...
  for r in select * from public.m_devices where m_uid = p_merge loop
    if exists (select 1 from public.m_devices where m_uid = p_keep and device_key = r.device_key) then
      update public.m_devices
      set last_seen_at = greatest(last_seen_at, r.last_seen_at), last_app_id = coalesce(r.last_app_id, last_app_id),
          last_org_id = coalesce(r.last_org_id, last_org_id), metadata = metadata || r.metadata
      where m_uid = p_keep and device_key = r.device_key;
  ...
  on conflict (m_uid, app_id, org_id) do update
    set first_seen_at = least(public.m_person_apps.first_seen_at, excluded.first_seen_at),
