...
//
// Takes a batch of messages from one external AI provider and lands them
// in `ai_context`, idempotently: an ingestion receipt keyed on
// (org, provider, idempotency_key) means replaying the same batch is a
// no-op rather than a duplicate, and message-level conflict handling
// means a partially-retried batch converges.
//
// IDENTITY — CHANGED 2026-09-07
// ----------------------------
// Same fix as context-query, and the same reasoning: the caller was
// derived by base64-decoding the JWT payload and trusting `sub`, which
// is a claim rather than a verified fact. Not exploitable as deployed —
// the gateway's verify_jwt rejected forged tokens before they reached
// this code — but this function WRITES, and the identity it read was
// also being stamped into the receipt as `ingested_by`. An identity
// worth recording is an identity worth verifying, or the audit trail is
// only as true as the least careful caller.
//
// It now asks the issuer through `verifyCaller`. The org membership
