...
async function objectiveById({ orgId, objectiveId }) {
  const params = new URLSearchParams({ id: `eq.${objectiveId}`, org_id: `eq.${orgId}`, select: '*', limit: '1' });
  const { body: rows = [] } = await rest(`ops_objectives?${params.toString()}`);
  return rows[0] || null;
}

export async function createObjective({ objectiveId, orgId, name, description = '', priority = 50, successMetric = {}, scope = {} } = {}) {
  ...
  const { body: rows = [] } = await rest(endpoint, { method: 'POST', headers: { Prefer: prefer }, body: JSON.stringify(row) });
...
export async function updateObjective({ orgId, objectiveId, name, description = '', priority = 50, successMetric = {}, scope = {} } = {}) {
  ...
  const params = new URLSearchParams({ id: `eq.${objectiveId}`, org_id: `eq.${orgId}`, status: 'eq.active' });
  const { body: rows = [] } = await rest(`ops_objectives?${params.toString()}`, {
    method: 'PATCH',
