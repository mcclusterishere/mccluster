...
export async function objectiveSynthesis(job) {
  ...
  if (validDecisionCheckpoint(checkpoint)) {
    ...
    checkpoint = { schema_version: '2', source, synthesis, usage, evidence: evidenceMeta };
  ...
  if (synthesis.action === 'ignore') {
    return { executor: 'objective_synthesis:v2', model: MODEL, action: 'ignore', confidence: synthesis.confidence, reason: synthesis.reason, source, evidence: evidenceMeta, retry_safe: true };
  }
  ...
  return {
    executor: 'objective_synthesis:v2', model: MODEL, action: synthesis.action, confidence: synthesis.confidence, reason: synthesis.reason, source,
