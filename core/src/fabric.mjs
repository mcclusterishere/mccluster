...
export function hashEnvelope(envelope) {
  const canonical = { schema_version: envelope.schema_version ?? 1, event_id: envelope.event_id, org_id: envelope.org_id, trace_id: envelope.trace_id, kind: envelope.kind, origin_node: envelope.origin_node, occurred_at: envelope.occurred_at, payload: envelope.payload ?? {} };
...
export function createEnvelope({ org_id, trace_id, kind, payload = {}, event_id, occurred_at } = {}) {
  ...
  const envelope = { schema_version: 1, event_id: event_id || crypto.randomUUID(), org_id, trace_id: trace_id || crypto.randomUUID(), kind, origin_node: NODE, occurred_at: occurred_at || new Date().toISOString(), payload };
  return { ...envelope, content_hash: hashEnvelope(envelope) };
}

async function existingEvent(eventId) {
  const rows = await sb(`fabric_events?event_id=eq.${encodeURIComponent(eventId)}&select=*&limit=1`);
  return rows?.[0] || null;
}
...
export async function persistEvent(envelope) {
  ...
  const rows = await sb('fabric_events', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify(envelope) });
  return { event: rows?.[0] || envelope, duplicate: false };
}

export async function markAck(eventId, node = NODE) {
  const now = new Date().toISOString();
  await sb('fabric_receipts?on_conflict=event_id,node', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ event_id: eventId, node, state: 'acked', first_seen_at: now, acked_at: now, attempts: 1, updated_at: now }) });
}

export async function ackOutbox(eventId, targetNode = NODE) {
  const now = new Date().toISOString();
  await sb(`fabric_outbox?event_id=eq.${encodeURIComponent(eventId)}&target_node=eq.${encodeURIComponent(targetNode)}`, { method: 'PATCH', body: JSON.stringify({ status: 'acked', acked_at: now, locked_at: null, locked_by: null, last_error: null, updated_at: now }) });
}

export async function relayToEdge(envelope) {
  if (!EDGE_TOKEN) return false;
  const response = await fetch(`${EDGE_URL}/v1/fabric/events`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-mccluster-fabric-token': EDGE_TOKEN, 'x-mccluster-fabric-node': NODE }, body: JSON.stringify(envelope) });
...
export async function drainOutbox({ limit = 50 } = {}) {
  const now = new Date().toISOString();
  const rows = await sb(`fabric_outbox?target_node=eq.${NODE}&status=in.(pending,leased)&next_attempt_at=lte.${encodeURIComponent(now)}&order=created_at.asc&limit=${limit}&select=*`);
  let acked = 0;
  for (const row of rows || []) {
    const eventRows = await sb(`fabric_events?event_id=eq.${encodeURIComponent(row.event_id)}&select=*&limit=1`);
    const event = eventRows?.[0];
    if (!event) continue;
    if (hashEnvelope(event) !== event.content_hash) {
      await sb(`fabric_outbox?event_id=eq.${encodeURIComponent(row.event_id)}&target_node=eq.${NODE}`, { method: 'PATCH', body: JSON.stringify({ status: 'dead', attempts: (row.attempts || 0) + 1, last_error: 'content_hash mismatch', updated_at: now }) });
      continue;
    }
...
export async function reconcileFabric({ limit = 250 } = {}) {
  const events = await sb(`fabric_events?select=event_id,origin_node&order=created_at.desc&limit=${limit}`);
  for (const event of events || []) {
    for (const target of ['cloudflare', 'ovh']) {
      if (event.origin_node === target) continue;
      await sb('fabric_outbox?on_conflict=event_id,target_node', { method: 'POST', headers: { Prefer: 'resolution=ignore-duplicates,return=minimal' }, body: JSON.stringify({ event_id: event.event_id, target_node: target, status: 'pending' }) });
    }
  }
