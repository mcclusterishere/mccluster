...
export function normalizeObjectiveSynthesis(value, { existingObjectiveIds = [] } = {}) {
  ...
  if (action === 'update') {
    ...
    return { action, objective_id: objectiveId, name, description, priority, success_metric: successMetric, confidence: score, reason };
  }

  return { action: 'create', name, description, priority, success_metric: successMetric, confidence: score, reason };
}
