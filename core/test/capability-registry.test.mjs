...
test('lists active and planned capabilities without pretending planned work is callable', async () => {
  const registry = new CapabilityRegistry({ toolRegistry: fakeTools(['primary.health']), catalog });
  ...
  assert.deepEqual(health.providers, ['primary']);
  assert.equal(video.available, false);
});

test('resolves the highest-priority available implementation', async () => {
  const registry = new CapabilityRegistry({ toolRegistry: fakeTools(['primary.health', 'fallback.health']), catalog });
  const resolved = await registry.resolve('system.health');
  assert.equal(resolved.binding.id, 'provider.primary');
});

test('requirements can route to a different implementation', async () => {
  const registry = new CapabilityRegistry({ toolRegistry: fakeTools(['primary.health', 'fallback.health']), catalog });
  const resolved = await registry.resolve('system.health', { requirements: { authenticated: false } });
  assert.equal(resolved.binding.id, 'provider.fallback');
});

test('capability calls delegate through the normalized tool registry', async () => {
  const tools = fakeTools(['primary.health']);
  ...
  assert.deepEqual(tools.calls, [{ name: 'primary.health', args: { probe: true } }]);
});
